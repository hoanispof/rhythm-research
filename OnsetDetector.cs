...
namespace RhythmTool
{
    ...
    public class OnsetDetector : Analysis<Onset>
    {
        ...
        [Range(2, 512), Tooltip("The size of the buffer determines the minimum time between detected onsets and how much of the surrounding data is used for calculating the threshold.")]
        ...
        public override void Process(float[] samples, float[] magnitude, int frameIndex)
        {
            ...
            /*
            if (name.Equals("Easy Onsets"))
            {
                Debug.Log(frameIndex + "_" + (frameIndex - bufferSize / 2) + "_" + current + "_" + max + " - " + ((current == max)?("Peek" + "_" + FrameIndexToSeconds(frameIndex - bufferSize / 2)) : "") + "\n"
                    + "sample=" + sample + "\n"
                    + "frameIndex % bufferSize=" + (frameIndex % bufferSize) + "\n"
                    //+ "samples=" + FuncArray.ArrayPreview(samples) + "\n"
                    //+ "magnitude=" + FuncArray.ArrayPreview(magnitude) + "\n"
                    );
            }
            */
            if (current == max)
            {
                ...
                //if (peak > avg + threshold * (peak - avg))
        ...
        float SpectralDifference(float[] magnitude)
        {
            float diff = 0;
